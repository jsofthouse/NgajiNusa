...
class UpdateReferralAgentRequest extends FormRequest
{
    ...
    {
        // route model binding: {referralAgent}
        $agentId = $this->route('referralAgent')?->id;

        return [
            'nama' => ['required', 'string', 'min:3', 'max:100'],
            'email' => ['required', 'email:rfc,dns', 'max:150', Rule::unique('referral_agents', 'email')->ignore($agentId)],
            'whatsapp' => ['required', 'regex:/^(\+?62|0)8[0-9]{8,12}$/', Rule::unique('referral_agents', 'whatsapp')->ignore($agentId)],
