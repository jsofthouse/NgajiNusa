...
- Landing page publik + modal pendaftaran (frontend). Tombol "Pilih Paket" di tiap card pricing auto-isi field Paket di modal (2026-07-14, lihat §11).
...
- **Nama fitur:** Revisi Landing Page & Pengaturan (hasil rapat pra-rilis 2026-07-21 + keputusan susulan 2026-07-22) — rewording fitur, paket baru Diamond, footer, tombol CTA, hero-stats, section Alur/Cara Daftar, e-wallet dihapus dari pengaturan.
- **Tanggal:** 2026-07-22.
- **Perubahan utama:** Landing page (`pages/home.blade.php`): (1) 3 fitur di-reword — "Guru Bersertifikat" → "Guru Bersertifikat/Lulusan Ponpes" + desc lulusan pondok pesantren, "Bayar Mudah" jadi "Pembayaran via QRIS atau transfer bank" (e-wallet dihapus), "Sertifikat" → "Sertifikat Kelulusan". (2) Paket baru **Diamond** (Rp1,5jt/bulan, 12x pertemuan 60 menit — sama seperti Premium — + "Guru pilihan (tahsin/tajwid/qori)") ditambahkan sebagai paket termahal, masuk `Murid::PAKET_OPTIONS` & `TransaksiService::PAKET_PRICES` (otomatis berlaku di validasi & dropdown admin murid karena loop dari konstanta). (3) Footer sosmed sisa Instagram+WhatsApp (YouTube dihapus); footer layanan urutan baru Iqra/Tahsin/Tajwid/Qori (Hafalan dihapus dari footer saja, `Murid::LEVEL_OPTIONS` backend tidak disentuh). (4) Tombol CTA "Daftar Sekarang" di-fix bg hijau + font putih via inline style — ditemukan CSS `.cta .btn-primary` sebenarnya override ke bg putih/font hijau (kebalikan asumsi rapat), kalau font putih diterapkan mentah di atas itu jadi tidak kelihatan, jadi di-fix inline supaya hasil akhir sesuai maksud owner. Tombol "Daftar" di navbar ternyata kena bug serupa (selector `.nav-links a` specificity-nya lebih tinggi dari `.btn-primary`, font jadi abu-abu bukan putih) — ditemukan dari feedback owner setelah cek visual, di-fix inline `color:var(--white)` juga. (5) Hero-stats di-hide pakai inline `style="display:none"` (bukan class `d-none` — landing page ternyata tidak load Bootstrap CSS sama sekali). (6) Section baru "Alur/Cara Daftar" (4 langkah: Isi Formulir → Chat WA Admin → Bayar QRIS/Transfer → Kelas Dimulai) ditambahkan setelah Paket, sebelum Testimonial — reuse class `.features`/`.features-grid`/`.feature-card` existing, tanpa CSS baru. Testimoni tidak disentuh. Pengaturan admin (`admin/pengaturan.blade.php`): toggle metode pembayaran sisa Transfer Bank + QRIS (E-Wallet & Kartu Kredit dihapus) — halaman ini murni mock UI (route closure, tanpa controller), tidak ada backend metode pembayaran yang perlu disesuaikan.
- **File utama yang diubah:** `resources/views/pages/home.blade.php`, `resources/views/admin/pengaturan.blade.php`, `app/Models/Murid.php`, `app/Services/TransaksiService.php`.
- **Dampak:** Landing page & pengaturan sesuai keputusan pra-rilis, siap rilis. Tidak ada perubahan skema DB (`paket` tetap kolom string bebas), tidak ada perubahan route/method existing.
- **Known limitation:** Referral Komisi Agent (Fase 2+, 5% dari harga paket) masih belum diimplementasikan — tetap PENDING di `docs/todo.md`, di luar scope eksekusi ini.

### Riwayat sebelumnya

- **Revisi Landing Page Paket Pengajian — paket Group** (2026-07-15): paket baru "Group" (Rp150K/bulan) + benefit baru di paket privat (lihat §18 Changelog untuk detail lengkap).
...
8. **Referral — Komisi Agent (Fase 2+)** — 5% dari harga paket (nominal sementara, belum fix), trigger saat pembayaran terverifikasi. Masih PENDING, belum diimplementasikan. Detail di `docs/todo.md`. (Revisi landing page & pengaturan dari rapat pra-rilis 2026-07-21 sudah dieksekusi 2026-07-22 — lihat §11.)
...
- **Harga paket hanya di frontend** (home.blade) + `TransaksiService::PAKET_PRICES`, tidak ada sumber backend tunggal (tabel harga sungguhan).
- **Referral Komisi Agent (Fase 2+) belum diimplementasikan** — 5% dari harga paket, nominal belum fix. Lihat `docs/todo.md`.
...
| ---------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| 2026-07-22 | Revisi Landing Page & Pengaturan (eksekusi hasil rapat pra-rilis 2026-07-21 + keputusan susulan 2026-07-22) | 3 fitur di-reword (Guru Bersertifikat/Lulusan Ponpes, Bayar Mudah → QRIS/transfer bank tanpa e-wallet, Sertifikat Kelulusan). Paket baru Diamond (Rp1,5jt/bulan, 12 sesi sama Premium + "Guru pilihan tahsin/tajwid/qori") masuk `Murid::PAKET_OPTIONS` & `TransaksiService::PAKET_PRICES`. Footer sosmed sisa Instagram+WhatsApp, urutan layanan Iqra/Tahsin/Tajwid/Qori (Hafalan dihapus dari footer saja). Tombol CTA "Daftar Sekarang" di-fix bg hijau+font putih via inline style (CSS `.cta .btn-primary` ternyata override ke bg putih/font hijau — kalau diterapkan mentah font jadi tak kelihatan). Hero-stats di-hide inline `display:none` (bukan `d-none`, landing page tak load Bootstrap). Section baru "Alur/Cara Daftar" (4 langkah) ditambahkan setelah Paket sebelum Testimonial, reuse class existing. Pengaturan admin: toggle metode pembayaran sisa Transfer Bank+QRIS (E-Wallet & Kartu Kredit dihapus). File: `pages/home.blade.php`, `admin/pengaturan.blade.php`, `Murid.php`, `TransaksiService.php`. | Landing page & pengaturan siap rilis sesuai keputusan pra-rilis. Tidak ada perubahan skema DB/route/method existing. Referral Komisi Agent (Fase 2+) tetap belum diimplementasikan, terpisah dari scope ini. |
| 2026-07-21 | docs: Catatan Rapat Pra-Rilis (Revisi Landing Page & Referral Komisi) — sudah dieksekusi 2026-07-22 (lihat baris di atas), Referral Komisi masih PENDING | Rapat pra-rilis mencatat rencana revisi landing page (`pages/home.blade.php`): rewording section Fitur (Guru Bersertifikat → Guru Bersertifikat/Lulusan Ponpes, Bayar Mudah hapus e-wallet jadi QRIS/transfer bank, Sertifikat → Sertifikat Kelulusan), paket baru Diamond (Rp1,5jt/bulan, isi = Premium + opsi "Guru pilihan (tahsin/tajwid/qori)", jumlah sesi belum fix), footer (sosmed hapus YouTube sisa Instagram+WhatsApp, urutan layanan baru Iqra/Tahsin/Tajwid/Qori tanpa Hafalan), tombol CTA "Daftar Sekarang" (isu kontras font putih di atas bg putih, belum fix), hero-stats di-hide sementara, testimoni ditahan (tetap dummy), section baru Alur/Cara Daftar 4 langkah (posisi di landing page belum fix). Juga catatan roadmap Fase 2+: Referral Komisi Agent 5% dari harga paket (trigger saat pembayaran terverifikasi), opsi jangka panjang multi-line marketing (wacana, bukan requirement pasti). Keputusan tertunda: nasib e-wallet toggle di `admin/pengaturan.blade.php`. Detail lengkap di `docs/todo.md` (section "Revisi Tertunda — Rapat Pra-Rilis"). | **Tidak ada kode yang diubah** — murni pencatatan hasil rapat. Semua poin berstatus PENDING, menunggu eksekusi & beberapa klarifikasi lanjut dari owner (Diamond sesi, posisi section, warna CTA, nasib e-wallet toggle). |
